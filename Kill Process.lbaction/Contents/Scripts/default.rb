...
item = {}
item['title'] = ARGV.length.to_s + ' arguments passed'
items.push(item)
...
items = processes.map do | process |
    ...
    {
      title: "#{processName}#{matchedArgs.join(" ")}",
      subtitle: "#{processCpu}% CPU @ #{processPath}",
      icon: iconValue,
      action: 'kill_process.rb',
      actionArgument: processId
    }
